...
# 1. Engines and connection strings
...
# 2. Autoloading Tables from a database
...
# 4. Selecting data from a Table: raw SQL
from sqlalchemy import create_engine
engine = create_engine('sqlite:///census.sqlite')
...
# 5. Selecting data from a Table with SQLAlchemy
# Import select
...
# 2. Filter data selected from a Table - Simple
...
# 3. Filter data selected from a Table - Expressions
...
# 4. Filter data selected from a Table - Advanced
# Import and_
...
# 6. Ordering in descending order by a single column
# Import desc
...
# 7. Ordering by multiple columns
...
# 11. ResultsSets and pandas DataFrames
# import pandas
import pandas as pd
...
# 12. From SQLAlchemy results to a plot
...
# 1. Calculating a difference between two columns
...
# 2. Determining the overall percentage of women
...
# 3. Automatic joins with an established relationship
...
# 6. Using alias to handle same table joined queries
...
# 7. Leveraging functions and group_bys with hierarchical data
# Make an alias of the employees table: managers
managers = employees.alias()
...
# 8. Working on blocks of records
...
# 1. Creating tables with SQLAlchemy
...
new_results = connection.execute(select([state_fact]).where(state_fact.columns.name == 'New York')).fetchall()
